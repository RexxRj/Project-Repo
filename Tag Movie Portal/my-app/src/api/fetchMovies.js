export const fetchMovies = async (searchText, moviesCallback, errorCallback, finallyCallback) => {
    try {
        ...
        const response = await fetch(`http://www.omdbapi.com/?s=${searchText}&apikey=85b44c32&type=movie`);
...
const fetchMovieDetails = async (id,errorCallback) => {
    try {
        const response = await fetch(`http://www.omdbapi.com/?i=${id}&plot=full&apikey=85b44c32`);
        const data = await response.json();
